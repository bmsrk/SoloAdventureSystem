...
/// <summary>
/// Tests for MaIN.NET adapter to verify AI text generation is working correctly.
/// Uses a shared fixture to avoid downloading/loading the model multiple times.
/// </summary>
[Collection("MaIN.NET Collection")]
public class MaINAdapterTests
{
    private readonly ITestOutputHelper _output;
    private readonly MaINFixture _fixture;

    public MaINAdapterTests(ITestOutputHelper output, MaINFixture fixture)
    {
        _output = output;
        _fixture = fixture;
    }
...
/// <summary>
/// Shared fixture for MaIN.NET tests - initializes model once for all tests
/// </summary>
public class MaINFixture : IDisposable
{
    ...
    {
        Console.WriteLine("????????????????????????????????????????????????????????????");
        Console.WriteLine("? Initializing MaIN.NET Test Fixture                      ?");
        Console.WriteLine("????????????????????????????????????????????????????????????");
        Console.WriteLine();
        ...
        var logger = new XunitLogger<MaINAdapter>(new ConsoleTestOutputHelper());
    ...
    {
        Console.WriteLine();
        Console.WriteLine("Disposing shared MaIN.NET adapter...");
        Adapter?.Dispose();
...
/// <summary>
/// Collection definition for MaIN.NET tests
/// </summary>
[CollectionDefinition("MaIN.NET Collection")]
